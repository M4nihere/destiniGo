    ...
    {images.slice(0, 6).map((src, index) => (
      ...
      >
        ...
        <Image
          ...
          layout="fill"
          objectFit="cover"
          className="rounded-xl cursor-pointer transition-transform duration-300 hover:scale-110"
          onClick={() => {
            setShowGallery(true);
    ...
    {certificates.map((src, index) => (
      ...
      {selectedCertificate && (
        <div className="fixed top-[-65px] left-0 bg-black bg-opacity-90 backdrop-blur-md flex items-center justify-center z-50 w-full h-screen transition-opacity duration-300 ease-in-out">
          ...
          <Image src={selectedCertificate} alt="Selected Certificate" width={500} height={350} className="object-cover rounded-lg shadow-lg" />
        </div>
      )}
    </div>
  );
